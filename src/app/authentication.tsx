...
export default function AuthenticationScreen() {
	...
	// Setter opp Google Auth request
	...
	// Vanlig e-post/pw innlogging
	const submit = async () => {
		// Sjekker input basert på modus
		...
		try {
			...
			// Send bruker til beskyttet område
	...
	useEffect(() => {
		...
		if (Platform.OS === "web") {
			const handleMessage = async (event: MessageEvent) => {
				...
				// Har token – prøv å logge inn i Firebase
				if (idToken) {
					try {
			...
			// Legger til lytter når skjermen rendres
			window.addEventListener("message", handleMessage);

			// Fjerner lytter ved unmount
			return () => {
				window.removeEventListener("message", handleMessage);
			};
		}

		// Native, bruker response direkte
